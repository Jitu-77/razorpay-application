...
import com.jitu.razorpay_application.common_lib.exceptions.IdempotencyConflictException;
...
public class IdempotencyFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        // now need to intercept the api req hence we will place this just after merchant and
        // apiKeyFilter as we need the merchant ID
        UUID merchantId = merchantContext.getMerchantId();
        String key = merchantId != null ? merchantId+":"+rawKey : rawKey;

        //now we need to make this thread safe
        ...
        if (!claimed) {
            ...
            if (existing.isPresent() && !IdempotencyStore.IN_PROGRESS.equals(existing.get())) {
                ...
                //return the same value to its user using the replay
        ...
        try {
            ...
            if (status < 400 && bodyBytes.length > 0) {
                ...
                idempotencyStore.store(key, stored, COMPLETED_TTL); //long time after which we can retry
